...
// extract the four lowest elements
        nth_element (coll.begin(),
                     coll.begin()+3, coll.end());

// beginning of range // element that should be sorted correctly // end of range
...
// print them
        cout << "the four highest elements are: "; // partially ordered just enough to tell you the nth is correct

        copy (coll.cbegin(), coll.cbegin()+4, ostream_iterator<int>(cout," ")); cout << endl;
}
